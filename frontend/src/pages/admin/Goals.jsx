...
import { getAllUsers, unlockGoal } from'@/api/adminApi';
...
function GoalsPage() {
 ...
 // Fetching all users + their goals — admin sees all goal sheets
 ...
 // Flatten goals from users data
 const users = data || [];
 const allGoals = users.flatMap((user) =>
 (user.goalSheet?.goals || []).map((goal) => ({
 ...goal,
 ...
 // Client-side filters for name and status
 const filteredGoals = allGoals.filter((goal) => {
 if (nameSearch && !goal.employeeName?.toLowerCase().includes(nameSearch.toLowerCase())) {
 return false;
 }
 ...
 <th className="px-6 py-4 text-left font-semibold text-gray-500">Goal Title</th>
 <th className="hidden px-6 py-4 text-left font-semibold text-gray-500 md:table-cell">Thrust Area</th>
 <th className="hidden px-6 py-4 text-left font-semibold text-gray-500 lg:table-cell">UoM</th>
 <th className="hidden px-6 py-4 text-center font-semibold text-gray-500 lg:table-cell">Weightage</th>
 <th className="px-6 py-4 text-left font-semibold text-gray-500">Status</th>
 <th className="px-6 py-4 text-center font-semibold text-gray-500">Locked</th>
 ...
 <div className="font-medium text-gray-900">{goal.employeeName}</div>
 <div className="text-xs text-gray-500">{goal.employeeDepartment}</div>
 </td>
 ...
 <td className="hidden px-6 py-4 text-gray-600 lg:table-cell">{goal.uomType || goal.uom}</td>
